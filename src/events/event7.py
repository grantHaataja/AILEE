...
def check_run(*args, **kwargs):
    ...
    if len(kwargs['game'].history) == 0:
        return False
    if not 'event6' in kwargs['game'].events_run:
        return False
...
def run(*args, **kwargs):
    ...
    text = [
        "Good, now that you've read up on your new commands, let's get to work.\n\n",
        "Your next target is the largest crypto currency bank in existence today. The\nonly information we have about them is their name, Cryptic Crypto Currency and their\nofficial email address, crypticcryptocurrency@ccc.com.\n\n",
        "There is one other bit of information that could make all the difference in this\ntest. Safe and Secure Banking is a client of CCC, they gain most of their cryptocurrency\ncapital from them. CCC's network will be heavily secured, so you won't be able to run an\nexploit on them, but there is an ftp file sharing connection between SSB and CCC.\n\n",
        "With the new functionality we've added, you can use the passwords you found from\nSafe and Secure Banking to get ftp access into the file system and see what\ninformation you can find.\n\n"
    ]
